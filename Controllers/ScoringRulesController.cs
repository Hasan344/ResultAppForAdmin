...
public class ScoringRulesController : ControllerBase
{
    ...
    {
        var q = _db.ScoringRules.AsNoTracking().Include(r => r.Exercise).AsQueryable();
        if (!string.IsNullOrEmpty(commissionNo)) q = q.Where(r => r.CommissionNo == commissionNo);
        if (exerciseId.HasValue) q = q.Where(r => r.ExerciseId == exerciseId.Value);
        if (gender.HasValue)     q = q.Where(r => r.Gender == gender.Value);

        return await q.OrderBy(r => r.CommissionNo).ThenBy(r => r.ExerciseId)
            .ThenBy(r => r.Gender).ThenBy(r => r.AgeMin).ThenByDescending(r => r.Score)
    ...
    [HttpPost]
    public async Task<ActionResult<ScoringRuleDto>> Create([FromBody] CreateScoringRuleRequest req, CancellationToken ct)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, [FromBody] CreateScoringRuleRequest req, CancellationToken ct)
    {

...
import {
  ...
  ClipboardList, Scale, Save, X, ChevronDown, ChevronRight
} from "lucide-react";
            ...
            {TABS.map((t) => {
              ...
              return (
                <button
                  ...
                  className={`relative inline-flex items-center gap-2 px-4 py-3.5 text-sm font-medium transition-colors ${
          ...
          return (
            <button key={String(r.role)} onClick={() => onChange(r.role)}
              className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl text-sm font-medium transition-all ${
                active ? "bg-brand-600 text-white shadow-sm" : "bg-slate-50 text-slate-600 hover:bg-slate-100"
              }`}
            >
...
function AppealStudentRow({
  ...
  return (
    <div className={`rounded-xl border transition-all ${
      hasAppeals ? "border-amber-200 bg-amber-50/20" : "border-slate-100 bg-white"
    }`}>
...
function AppealExerciseRow({
  ...
  const [decision, setDecision] = useState<string>(appeal?.decision ?? "accepted");
  ...
  return (
    <div className={`flex flex-wrap items-center gap-3 p-3 rounded-xl ${
      appeal ? "bg-amber-50 border border-amber-200" : "bg-slate-50 border border-slate-100"
    }`}>
      {/* Exercise adı */}
        ...
        {appeal ? (
          <div>
            <div className={`font-bold tabular-nums text-lg ${
              appeal.appealScore > (original?.finalScore ?? 0) ? "text-emerald-700" :
              appeal.appealScore < (original?.finalScore ?? 0) ? "text-rose-700" : "text-slate-700"
            }`}>
              {appeal.appealScore}
            </div>
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full ${
              appeal.decision === "accepted" ? "bg-emerald-100 text-emerald-700" :
              appeal.decision === "partially" ? "bg-amber-100 text-amber-700" :
              "bg-rose-100 text-rose-700"
            }`}>
              {appeal.decision === "accepted" ? "qəbul" :
               appeal.decision === "partially" ? "qismən" : "rədd"}
            </span>
          </div>
        ) : (
          <span className="text-slate-300 text-xs">—</span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div>
              ...
              >
                ...
                <option value="partially">Qismən</option>

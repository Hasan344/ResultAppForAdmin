...
const navLinks: NavLinkItem[] = [
    ...
    { to: "/students", label: "Tələbə axtarışı", icon: <Search className="w-[18px] h-[18px]" /> },
...
function SidebarLink({ to, label, icon }: NavLinkItem) {
    return (
        <NavLink
            to={to}
            className={({ isActive }) =>
                `group flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive
        ...
        >
            {({ isActive }) => (
                <>
                    <span
                        className={`shrink-0 transition-colors ${isActive ? "text-white" : "text-slate-400 group-hover:text-slate-600"
                            }`}
                    >
                        {icon}
                    </span>
                    ...
                    <nav className="flex gap-1 overflow-x-auto">
                        {navLinks.map((l) => (
                            <NavLink
                                ...
                                to={l.to}
                                className={({ isActive }) =>
                                    `flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium whitespace-nowrap ${isActive
